...
import { submitAnomalyInterceptionTeam } from "@/app/(web)/anomaly-interception/actions";
import GenericSubmitTeam from "@/components/base/generic-submit-team";
import CharacterCard from "@/components/character-card";
import { z } from "zod";
import { toast } from "@/components/ui/use-toast";
import { ToastAction } from "@/components/ui/toast";
import { AnomalyInterceptionTeamSubmissionSchema } from "@/lib/types/zod";
import { Tables } from "@/lib/types/database.types";

interface AnomalyInterceptionSubmitTeamProps {
  modeId: string;
  modeName: string;
  boss: Tables<"bosses">;
...
export default function AnomalyInterceptionSubmitTeam({
  modeId,
  modeName,
  boss,
  versions,
  onClose,
}: AnomalyInterceptionSubmitTeamProps) {
  const { user } = useAuth();
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      for (let i = 0; i < teams.length; i++) {
        ...
        const validatedData =
          AnomalyInterceptionTeamSubmissionSchema.parse(submissionData);

        const result = await submitAnomalyInterceptionTeam(validatedData);
